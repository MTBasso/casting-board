...
| Where it lives | `src/ui/season/`, mounted behind `#season` in `main.tsx`; the old league-manager `App` is untouched and stays the default | Small vertical slice, deployed alongside the current game rather than replacing it, per this file's own process note |
